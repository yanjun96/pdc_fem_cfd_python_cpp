...
T0      = 300;                   % K
RHO0    = P0/T0/R;               % kg/m^3
Kentr   = P0/RHO0^GAMMA;         % isentropic relation      IS THE K IN description
...
for k = 1:nstp
  ...
  % pressure and velocity
   if rem(k,4) == 0,           % plot every fourth time step 
    figure(5)
    subplot(311)
    plot(X,rho,'r'), title('C0=0.05, C2=0.25,  Density')
    subplot(312),plot(X,u,'b'), title('Velocity')
    subplot(313),plot(X,p,'m'), title('Pressure') 
    drawnow;
